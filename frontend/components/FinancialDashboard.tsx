...
import { Stock } from "@/data/financial-data";
...
export function FinancialDashboard() {
  ...
  >([
    { symbol: "AAPL", quantity: 50, avgPrice: 175 },
    { symbol: "MSFT", quantity: 30, avgPrice: 350 },
    { symbol: "GOOGL", quantity: 25, avgPrice: 130 },
  ]);
        ...
        const fetchData = async () => {
          try {
            const response = await fetch(
              "http://localhost:8000/portfolio/calculate",
              {
                method: "POST",
  ...
  const filteredStocks = FINANCIAL_STOCKS.filter((stock) => {
    const matchesSearch =
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  >
                    ...
                    {Array.from(new Set(FINANCIAL_STOCKS.map((s) => s.sector)))
                      .sort()
                      .map((sector) => (
                ...
                <Badge variant="secondary" className="text-[10px] font-mono">
                  {filteredStocks.length} of {FINANCIAL_STOCKS.length}
                </Badge>
              </div>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {[
                            ...
                            {
                              label: "Beta",
                              value: selectedStock.beta.toFixed(2),
                            },
                            {
                              label: "52W High",
                              value: formatCurrency(selectedStock.high52w),
                            },
                            {
                              label: "52W Low",
                              value: formatCurrency(selectedStock.low52w),
                            },
                          ].map((item) => (
                            <div
                              key={item.label}
                  ...
                  <Card className="border border-slate-200">
                    ...
                    <CardContent className="space-y-2">
                      {portfolioPositions.length > 0 ? (
                        portfolioPositions.map((position) => {
                          const stock = FINANCIAL_STOCKS.find(
                            (s) => s.symbol === position.symbol
                          );
                          if (!stock) return null;

                          const rsi = stock.technicalIndicators.rsi;
                          const macd = stock.technicalIndicators.macd;
