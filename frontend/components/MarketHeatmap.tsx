...
import { RefreshCw, Grid3x3, ScatterChart, TrendingUp, TrendingDown, Activity, BarChart3 } from "lucide-react";
...
export function MarketHeatmap() {
  ...
  const [selectedStock, setSelectedStock] = useState<StockHeatmapData | null>(null);
  ...
  const getSectorGradient = (avgChange: number) => {
    if (avgChange > 2) return "from-green-500/10 via-green-400/5 to-transparent";
      ...
      <Card className="border-0 shadow-xl bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800">
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-2xl">Loading Market Data...</CardTitle>
              <CardDescription>Fetching real-time stock information</CardDescription>
            </div>
          </div>
        </CardHeader>
      </Card>
  ...
  const totalVolume = heatmapData.reduce((sum, s) => sum + s.volume, 0);
  const advancers = heatmapData.filter(s => s.changePercent > 0).length;
  const decliners = heatmapData.filter(s => s.changePercent < 0).length;
  const avgChange = heatmapData.reduce((sum, s) => sum + s.changePercent, 0) / heatmapData.length;
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader className="relative z-10">
          ...
          <div className="grid grid-cols-4 gap-4 mt-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
              <div className="text-blue-200 text-sm font-medium mb-1">Total Stocks</div>
            ...
            <div className="bg-green-500/20 backdrop-blur-sm rounded-xl p-4 border border-green-400/30">
              ...
              <div className="text-3xl font-bold text-green-100">{advancers}</div>
              <div className="text-xs text-green-200 mt-1">{((advancers/heatmapData.length)*100).toFixed(0)}%</div>
            </div>
            <div className="bg-red-500/20 backdrop-blur-sm rounded-xl p-4 border border-red-400/30">
              ...
              <div className="text-3xl font-bold text-red-100">{decliners}</div>
              <div className="text-xs text-red-200 mt-1">{((decliners/heatmapData.length)*100).toFixed(0)}%</div>
            </div>
            <div className={cn(
      ...
      <Tabs defaultValue="grid" className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-md h-12 bg-slate-100 dark:bg-slate-800 p-1">
          <TabsTrigger value="grid" className="flex items-center gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700">
            <Grid3x3 className="h-4 w-4" />
            Grid View
          </TabsTrigger>
          <TabsTrigger value="chart" className="flex items-center gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700">
              ...
              <CardHeader className="pb-4 bg-gradient-to-r from-slate-50/50 to-transparent dark:from-slate-900/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-2xl font-bold">{sector.name}</CardTitle>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">Sector Average</div>
                  </div>
                </div>
              </CardHeader>
                  ...
                  {sector.stocks.map((stock) => (
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="text-sm opacity-90 mb-2">₹{stock.price.toFixed(2)}</div>
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 shadow-xl">
                        ...
                        <div className="mt-1">Volume: {(stock.volume / 1000000).toFixed(2)}M</div>
                        <div className="mt-1">MCap: ₹{(stock.marketCap / 10000000).toFixed(0)}Cr</div>
          ...
          <Card className="border-0 shadow-xl bg-gradient-to-br from-slate-50 to-white dark:from-slate-900 dark:to-slate-800">
            <CardContent className="pt-6">
              <div className="flex items-center justify-center gap-6 flex-wrap text-sm">
                <span className="text-muted-foreground font-semibold">Performance Scale:</span>
                {[
                  { label: "> +3%", color: "from-green-600 to-green-700" },
                  { label: "+1 to +3%", color: "from-green-500 to-green-600" },
                  { label: "0 to +1%", color: "from-green-400 to-green-500" },
                  ...
                  <div key={idx} className="flex items-center gap-2">
                    <div className={cn("w-6 h-6 rounded-md shadow-md bg-gradient-to-br", item.color)}></div>
        ...
        >
          <Card className="max-w-md w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <CardHeader className="bg-gradient-to-br from-blue-500 to-purple-600 text-white">
              ...
              <CardDescription className="text-blue-100">{selectedStock.name}</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">{(selectedStock.volume / 1000000).toFixed(2)}M</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">Market Cap</span>
                  <span className="font-semibold">₹{(selectedStock.marketCap / 10000000).toFixed(0)} Cr</span>
                </div>
                <div className="flex justify-between items-center">
