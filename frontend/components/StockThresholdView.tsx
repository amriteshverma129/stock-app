...
export function StockThresholdView({ symbol }: { symbol: string }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const historicalData: ThresholdData[] = historyResponse.data.priceHistory.map(
        (item: any) => ({
          date: item.date,
          price: item.close,
        })
      ...
      const predictionData: ThresholdData[] = predictionResponse.data.predictions
        .slice(0, 30)
        .map((item: any) => ({
          date: item.date,
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                ₹
                {Math.max(...data.filter((d) => !d.predicted).map((d) => d.price)).toFixed(
                  2
                )}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                ₹
                {Math.min(...data.filter((d) => !d.predicted).map((d) => d.price)).toFixed(
                  2
                )}
              </span>
            </div>
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`font-semibold ${
                    ...
                    data[data.length - 1].predicted > currentPrice
                      ? "text-green-600"
                      : "text-red-600"
                  }`}
                >
