...
export function MarketScatterChart({ stocks, sectors }: MarketScatterChartProps) {
  const {
    showTooltip,
    hideTooltip,
    tooltipData,
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Market Cap (X-axis) vs Performance (Y-axis) • Size = Volume • Color = Sector
          </CardDescription>
        </CardHeader>
        <CardContent className="flex justify-center">
          <div style={{ position: "relative" }}>
                ...
                {stocks.map((stock, i) => {
                  ...
                  return (
                    <Circle
                      ...
                      cy={cy}
                      r={r}
                      fill={sectorColors[stock.sector] || sectorColors['Unknown']}
                      fillOpacity={0.7}
            ...
            {tooltipData && (
              ...
              >
                <div>
                  <div className="font-bold text-sm mb-1">{tooltipData.symbol}</div>
                  <div className="text-xs opacity-80 mb-2">{tooltipData.name}</div>
                  <div className="space-y-1">
                    <div>Price: ₹{tooltipData.price.toFixed(2)}</div>
                    <div className={tooltipData.changePercent >= 0 ? 'text-green-400' : 'text-red-400'}>
                      Change: {formatPercent(tooltipData.changePercent)}
                    </div>
                    <div>Volume: {(tooltipData.volume / 1000000).toFixed(2)}M</div>
                    <div>Market Cap: ₹{tooltipData.marketCap.toFixed(0)} Cr</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
            {Object.entries(sectorColors).filter(([sector]) => 
              sectors.some(s => s.name === sector)
            ).map(([sector, color]) => (
