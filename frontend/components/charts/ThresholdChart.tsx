...
export function ThresholdChart({
  ...
  const dateScale = scaleTime({
    domain: [Math.min(...data.map(getDate).map(d => d.getTime())), Math.max(...data.map(getDate).map(d => d.getTime()))],
    range: [0, xMax],
  });
  ...
  const threshold = thresholdValue || data.reduce((sum, d) => sum + getPrice(d), 0) / data.length;

  return (
    <div className="relative">
      <svg width={width} height={height}>
        {/* Gradients */}
        <LinearGradient id="area-gradient-green" from="#10b981" to="#10b981" toOpacity={0.2} />
        <LinearGradient id="area-gradient-red" from="#ef4444" to="#ef4444" toOpacity={0.2} />
        ...
        <Group left={margin.left} top={margin.top}>
          {/* Grid */}
          <GridRows scale={priceScale} width={xMax} strokeDasharray="3,3" stroke="#e5e7eb" />
          <GridColumns scale={dateScale} height={yMax} strokeDasharray="3,3" stroke="#e5e7eb" />
      ...
      <div className="flex items-center justify-center gap-6 mt-4 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600">Threshold (₹{threshold.toFixed(2)})</span>
        </div>
        <div className="flex items-center gap-2">
